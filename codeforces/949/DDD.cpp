...
struct lazySeg{
	...
	void init(int V){
		...
		for(int e=0;e<start*2;e++) lazy[e] = mk(0,0);
		...
		if(lazy[node].x){
			if(node<start){
				if(lazy[node].x==1){
					...
					if(lazy[node*2].x==2) lazy[node*2].y += lazy[node].y;
					else if(lazy[node*2].x==0) lazy[node*2] = lazy[node];
					if(lazy[node*2+1].x==2) lazy[node*2+1].y += lazy[node].y;
					else if(lazy[node*2+1].x==0) lazy[node*2+1] = lazy[node];
				}
			}
			if(lazy[node].x==1){
				arr[node] = 0;
			}else{
				arr[node] -= lazy[node].y*(ne-ns);
			}
			lazy[node] = mk(0,0);
		}
	}
...
void solve(int tc){
	...
	while(ll<=rr){
		if(ll==rr){
			lld tot = seg.sum(max(0ll,ll-cnt*d),min(rr,ll+cnt*d)+1);
			if(tot<b) x1++;
			break;
		}else{
			if(x1>=x2){
				...
				printf("%")
				if(tot<b) x1++;
				seg.add(max(0ll,ll-cnt*d),min(n-1,ll+cnt*d)+1,0,b);
				tot = seg.sum(max(0ll,rr-cnt*d),min(n-1,rr+cnt*d)+1);
				if(tot<b) x2++;
				seg.add(max(0ll,rr-cnt*d),min(n-1,rr+cnt*d)+1,1,b);
			}else{
				lld tot = seg.sum(max(0ll,rr-cnt*d),min(n-1,rr+cnt*d)+1);
				if(tot<b) x2++;
				seg.add(max(0ll,rr-cnt*d),n,1,min(n-1,rr+cnt*d)+1);
				tot = seg.sum(max(0ll,ll-cnt*d),min(n-1,ll+cnt*d)+1);
				if(tot<b) x1++;
				seg.add(max(0ll,ll-cnt*d),min(n-1,ll+cnt*d)+1,0,b);
			}
			ll++;
